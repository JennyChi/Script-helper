...
def update_server_content(conf_path):
...
def find_folder_name_end_with(folder_path, template):
    target_folder_name = ""
    dirContent = os.listdir(folder_path)
    for item in dirContent:
        if item.endswith(str(template)):
            target_folder_name = item
            break
    return target_folder_name
...
rc = "rc" + rc_number
log_file = "BAT-nc-jchi-" + version + "-" + rc + ".log"
...
# Add conf content
input_message = "Modify /etc/apps/search/local/indexes.conf files\n"
write_file(input_message, log_file_path)
conf_path = os.path.join(splunk_search_app, "indexes.conf")
...
update_server_content(conf_path)
...
input_message = "\n===== BAT scenario 2: searches =====\n"
write_file(input_message, log_file_path)
v1_url = "https://" + server_name + ":" + splunkd_port + "/services/data/indexes/" + vindex1
v1_count = Get_REST_endpoint_eventCount(v1_url, "admin", "changeme", rest_log_path)
input_message = "v1_count: " + str(v1_count) + "\n"
write_file(input_message, log_file_path)
v2_url = "https://" + server_name + ":" + splunkd_port + "/services/data/indexes/" + vindex2
...
write_file(input_message, log_file_path)
os.remove("%s" %(os.path.join(hot_bucket_path, "Sources.data")))
# Stop splunk 
os.system("""%s stop >> "%s" """ %(splunk_command, log_file_path))
# Detect Corrupt
os.chdir("%s" % splunk_bin)
to_execute = """%s cmd splunkd fsck scan --one-bucket --bucket-path="%s" --include-hots""" % (splunk_command, hot_bucket_path)
...
if not 'Corruption' in str(stderr):
    ...
    write_file(input_message, log_file_path)
# Start splunk to trigger online fsck to repair Sources.data
os.system("""%s start >> "%s" """ %(splunk_command, log_file_path))
time.sleep(300)
# Check Sources.data repaired and exists
tuple = hot_bucket_name.split("_v1_", 1)
bucket_id = tuple[1].strip()
bucket_end = "_" + bucket_id
warm_bucket_name = find_folder_name_end_with(index_homepath, bucket_end)
warm_bucket_path = os.path.join(index_homepath, warm_bucket_name)
input_message = "The repaired warm bucket is: " + warm_bucket_name  + "\n"
input_message = input_message + "warm_bucket_path = " + warm_bucket_path  + "\n"
write_file(input_message, log_file_path)
sources_data_path = os.path.join(warm_bucket_path, "Sources.data")
aTry = 1
maxTry = 3
while (aTry <= maxTry) and (os.path.exists(sources_data_path) == False):
    input_message = time.asctime(time.localtime()) + " " + str(aTry) + "try: online fsck repair is NOT completely. Wait 5 minutes\n"
    write_file(input_message, log_file_path)
    time.sleep(300)
    aTry += 1    
if aTry <= maxTry:
    input_message = "The online fsck repair is succesful. Good.\n"
else:
...
write_file(input_message, log_file_path)
scenario_result.append("===== BAT sceanrio 6: online fsck: " + result)

DS_app = "jchi_DS_" + version + "_" + rc + "_app"
...
4. Please check the deployment app exists on UI: %s
Please sing-off the jira as
*BAT OK to SignOff*
*Linux/Windows*
- Area: Indexing/Search/JSON&XML/Sparkline/Alerting/Report Acceleration/Online fsck
- Central site nonclustering linux/windows platform: %s
- Browser: 
""" %(savedsearch_name_rc, savedsearch_name_alert, DS_app, server_name)
write_file(input_message, log_file_path)
